...
public class SudokuController {
	...
	@FXML
	private ChoiceBox<String> boxLevel;
	...
	void doGenerate(ActionEvent event) {
		...
		String level = boxLevel.getValue();
		if (level != null) {
			if (level.compareTo("Easy") == 0)
				matrix = sg.nextBoard(levelEasy);
			else if (level.compareTo("Advanced") == 0)
				matrix = sg.nextBoard(levelEasy);
	...
	void doSolve(ActionEvent event) {
		if (matrix != null) {
			...
			int[][] risultato = ss.risolviSudoku(matrix);
			if (risultato != null)
				printMatrixOnScreen(risultato);
		}
	...
	@FXML
	void initialize() {
		assert boxLevel != null : "fx:id=\"boxLevel\" was not injected: check your FXML file 'Sudoku.fxml'.";
		...
		boxLevel.getItems().add("Easy");
		boxLevel.getItems().add("Advanced");
		boxLevel.getItems().add("Expert");
		boxLevel.getSelectionModel().selectFirst();
	}
